...
def newmodelrun(df,dropvars,model=mod_xgb,depvar='AD',reps=1,splits=2,shap=1,
    ...
    for reps in range(reps):
        ...
        dfscore=pd.DataFrame({'eid':eids,'score':yscore_tot})
        dfscore=pd.DataFrame(dfscore.groupby(['eid'])['score'].agg({'score':['mean','std']})).reset_index()
        dfscore.columns=['eid','mean_score','std_score']
